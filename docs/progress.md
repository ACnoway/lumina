...
- 使用现有本地依赖直接运行后端 Jest：**14 个测试套件、50 个测试全部通过**（认证验证码投递、管理员账户初始化、钱包并发/幂等与管理员调额审计、Provider RBAC/审计脱敏、管理员模型可见性、平台模型计费结构校验、管理端 RBAC、审计日志、MinIO 和生图队列）。
...
| 管理后台与审计 | 代码、单元测试和前端构建已验证 | `/admin` 已接入概览、用户/账本/余额调整、模型/供应商/上游映射、审计日志；平台模型表单按 `CHAT/IMAGE` 展示对应计费字段，后端 DTO/service 会拒绝错误计费结构；服务启动会按 `ADMIN_EMAIL` 幂等创建或修复管理员账户，真实数据仍需在 PostgreSQL、Redis 和 SMTP 环境做端到端验证。 |
...
- 新增平台模型计费校验单测 9 项；当前后端全量为 14 个测试套件、50 个测试通过，Nest/Next 生产构建通过。
