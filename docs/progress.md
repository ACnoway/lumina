...
## 本次模块：支付查单误入账回归修复（代码已更新，远程验证通过）

- 易支付查单不再把接口级 `code=1` 当作支付成功；只有明确的已支付订单状态才会返回 `SUCCESS`，未知/未支付状态保持 `PENDING`。
- 支付成功统一校验订单状态、人民币金额和币种；关闭/失败订单不能被后续成功查单恢复入账；充值仍使用 `payment:recharge:<orderNo>` 幂等键。
- 易支付查单兜底事件会标记为非签名回调，避免审计记录声称验签成功；前端支付状态轮询只读取订单，不触发主动查单或钱包变更。
- 新增支付服务与易支付回归测试，覆盖未付款查单、缺失状态、金额/币种不符、关闭订单、合法成功和无效签名；本地现有依赖下支付相关 14 项测试通过。
- 本地 pnpm 版本与仓库要求不一致，根级命令未执行；提交 `550dcbb` 已推送并在 `lch:/root/lumina` 使用 pnpm 8.15.0 完成全量测试（22 个测试套件 / 99 个测试）、生产构建和隔离 Compose 冒烟，测试资源已清理。现有冒烟覆盖认证、RBAC、钱包调整、聊天、生图、MinIO、历史和审计，但没有真实支付渠道交易；支付安全回归由后端 14 项单元/适配器测试覆盖。受影响期间的历史充值订单仍需逐笔与渠道对账。
...
- 个人中心已加入充值入口，支持支付方式、支付场景和具体渠道选择，并按订单状态轮询刷新余额；本次回归修复后轮询仅读取订单状态，不触发主动查单。
