...
- 消费金额按负向展示并使用红色；充值和退款使用正向展示；管理员调整按实际正负显示。前台统一展示“消费”“充值”“退款”“管理员调整”等中文，不直接展示 `CONSUME` 等枚举值。
- 管理后台最近账本同步使用中文交易类型和正确的消费红色显示，避免消费流水因后端金额存储为正数而错误显示为绿色。
- 新增用户账单 API 封装和 E2E 断言，覆盖用户看到管理员余额调整和服务消费流水的场景。
- 本地完整外部服务环境不可用；提交 `5dcc692` 已推送并在 `lch:/root/lumina` 通过 `pnpm test`（21 个测试套件 / 92 个测试）、`pnpm build` 和隔离 Compose `pnpm e2e:smoke`，测试资源已清理。
