...
- 远程 `lch:/root/lumina` 已拉取提交 `1001ed1`，启动隔离 Compose 环境并通过完整冒烟：验证码登录、管理员初始化、RBAC、用户状态启停、停用 JWT 拦截、余额调整、账本、Provider 滑动窗口限流、配置脱敏、聊天 SSE、生图队列、MinIO、历史和审计；测试完成后已清理隔离容器、网络和专用卷。
