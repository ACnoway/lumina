...
- 使用现有本地依赖直接运行后端 Jest：**11 个测试套件、32 个测试全部通过**（认证验证码投递、钱包并发/幂等与管理员调额审计、Provider RBAC/审计脱敏、管理员模型可见性、管理端 RBAC、审计日志、MinIO 和生图队列）。
...
- 历史远程验证曾通过；本次当前源码复核中，直接运行的后端单元测试为 11 suites / 32 tests，后端和前端构建通过。根级 pnpm 验证仍受本地 pnpm 版本不匹配阻塞。
