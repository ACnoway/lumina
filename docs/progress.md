...
- 本地没有 PostgreSQL、Redis、真实商户沙箱和公网回调环境；本地已完成 schema、类型、支付单元测试和构建验证，真实支付仍需在 `lch:/root/lumina` 使用沙箱凭据和公网回调做受控联调。
- 远程 `lch:/root/lumina` 已拉取提交 `50536c4`；重新生成 Prisma Client 后，根级 `lint`（0 error）、`test`（20 个 suite / 89 个测试）、`build` 均通过，隔离 Compose 冒烟也通过并已清理专用容器、网络和数据卷。该冒烟覆盖的是现有平台用户旅程，不等同于真实支付下单、回调或退款验收。
...
| 模块化支付渠道 V1 | 代码、支付单元测试、类型检查、构建和远程隔离环境已验证 | 已接入易支付 V1、支付宝官方、微信支付 API v3；前台展示可用渠道并由用户选择，不做优先级/权重或自动切换。远程尚未配置真实商户沙箱，因此真实下单、验签回调、重复回调、金额攻击和退款仍待补验。 |
