...
- 使用现有本地依赖直接运行后端 Jest：**15 个测试套件、57 个测试全部通过**（认证验证码投递、管理员账户初始化、钱包并发/幂等与管理员调额审计、Provider RBAC/审计脱敏与配置校验、管理员模型可见性、平台模型计费结构校验、管理端 RBAC、审计日志、MinIO 和生图队列）。
...
- 远程 `lch:/root/lumina` 已拉取提交 `26c8832`，启动隔离 Compose 环境并通过完整冒烟：验证码登录、管理员初始化、RBAC、配置脱敏、聊天 SSE、生图队列、MinIO、历史和审计。

### 当前交付阻塞项

1. **运行环境未就绪**：需要 PostgreSQL、Redis、MinIO、有效 `JWT_SECRET`、SMTP 及至少一个可用 AI Provider/模型后，才能做端到端验收。
2. **验证覆盖不足**：认证、聊天 SSE、生图队列和管理配置的 API 最小旅程已在 mock 环境通过；真实供应商、管理员余额调整/状态启停和浏览器关键流程仍未验证。
...
- 新增平台模型计费校验单测 9 项；当前后端全量为 15 个测试套件、57 个测试通过，Nest/Next 生产构建通过。
...
- 新增供应商配置校验单测 7 项；当前后端全量为 15 个测试套件、57 个测试通过，Nest/Next 生产构建通过。
