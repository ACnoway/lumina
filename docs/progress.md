...
- 使用现有本地依赖直接运行后端 Jest：**10 个测试套件、26 个测试全部通过**（钱包并发/幂等与管理员调额审计、Provider RBAC/审计脱敏、管理员模型可见性、管理端 RBAC、审计日志、MinIO 和生图队列）。
...
| 邮箱验证码登录、JWT、用户初始化钱包 | 代码已实现，**不可宣称可用** | 登录页与两个认证接口存在；邮件投递依赖有效 SMTP 配置，当前本机无 `.env`，默认示例 SMTP 不可用。详见 A-001。 |
...
| 管理后台与审计 | 代码、单元测试和前端构建已验证 | `/admin` 已接入概览、用户/账本/余额调整、模型/供应商/上游映射、审计日志；所有权限与真实数据仍需在 PostgreSQL、Redis 和管理员账号就绪后做端到端验证。 |
...
- 历史远程验证曾通过；本次当前源码复核中，直接运行的后端单元测试为 3 suites / 10 tests，后端和前端构建通过。根级 pnpm 验证仍受本地 pnpm 版本不匹配阻塞。
...
- 本机没有真实管理员、PostgreSQL、Redis 或 Provider 配置，因此尚未验证浏览器中的真实数据流和 mutation。

## 开发顺序总览

1. ~~项目脚手架~~ ✓
2. 用户模块 + 邮箱验证码登录（代码已实现；SMTP 端到端未验证，当前不可标为可用）
...
- **当前状态**：前端和认证接口均已实现并通过构建；没有有效 SMTP 配置时，发送验证码会返回 HTTP 400，不能标记为已可用。详见 `docs/known-issues.md` 的 A-001。
