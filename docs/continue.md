...
## 当前模块：支付查单误入账回归修复（已完成代码与远程验证）

- 创建订单只产生 `CREATED -> PENDING` 支付订单；余额只能由验签成功的支付通知，或服务端明确确认已付款的查单进入统一成功入口。
- 易支付 `code=1` 仅代表查单请求成功，不能代表订单已付款；未知状态、未付款状态和缺少支付金额/币种时必须保持不入账。
- 前端支付状态轮询只读取订单；主动同步接口保留给受控恢复使用，不能由浏览器创建订单后的自动轮询直接触发余额变化。
- 对外部署若由 Nginx 代理 `/api/`，`PAYMENT_NOTIFY_BASE_URL` 必须包含 `/api`，否则支付平台通知会落到前端而不是后端回调控制器。
- 本地支付相关 14 项测试通过；提交 `550dcbb` 已推送并由 `lch:/root/lumina` 使用 pnpm 8.15.0 完成全量测试（22 个测试套件 / 99 个测试）、生产构建和隔离 Compose `pnpm e2e:smoke`，结果通过且专用容器、网络和数据卷已清理。现有 smoke 未执行真实支付平台交易，部署前仍需配置并核对 `PAYMENT_NOTIFY_BASE_URL`，历史受影响订单需对账。
...
- 个人中心账单已接入用户自己的钱包流水，支持充值、消费、退款和管理员调整筛选、分页、余额快照和中文展示；消费金额显示为红色，管理员调整流水对用户可见。
...
- 后端复用 `WalletTransaction`，在 `GET /wallet/transactions` 增加 `type` 筛选参数，不新增账单表或迁移。
- 前端账单列表展示充值、消费、退款和管理员调整；`CONSUME` 等内部枚举只在接口层使用，用户界面统一显示中文。
- 消费流水因为钱包服务以正数保存扣减金额，前端按交易类型转换为负向并使用红色显示；余额快照仍展示交易完成后的余额。
- 消费固定显示 `-`，充值固定显示 `+`，即使金额为 0 也不省略符号；管理员后台账本采用相同的金额方向和颜色规则。
- 账单金额和交易后余额最多显示小数点后 4 位，超过第 4 位直接截断而不四舍五入；其他页面金额格式保持原有规则。
- E2E smoke 已加入普通用户读取管理员余额调整和消费流水的断言；`5dcc692` 已在 `lch:/root/lumina` 通过生产构建、21 个后端测试套件 / 92 个测试和完整隔离冒烟，容器、网络和专用卷已清理。
- 符号修正提交 `f17399a` 已在 `lch:/root/lumina` 通过最新生产构建。
- 四位小数截断提交 `8501f98` 已在 `lch:/root/lumina` 拉取并通过生产构建，前端类型检查和页面生成均通过。
