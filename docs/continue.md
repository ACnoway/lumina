...
> 更新日期：2026-09-19
> 验证提交：个人中心账单功能（本次分支 HEAD，远程验证待执行）
> 分支状态：个人中心账单已接入钱包流水；本地受 pnpm 版本和依赖目录限制，远程构建、完整 E2E 和浏览器验收待执行。
...
- E2E smoke 已加入普通用户读取管理员余额调整和消费流水的断言；推送后需在 `lch:/root/lumina` 重新执行远程验证。
