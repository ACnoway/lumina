...
### PAYMENT-001 · 易支付 GET 异步通知曾因仅支持 POST 导致充值不入账（已修复，待重新部署验证）

- **发现时间**：2026-09-19
- **问题**：用户完成易支付付款后，支付平台向 `/api/payments/notify/:channelId` 发起 GET 异步通知；后端此前只注册了 POST 路由，Nginx 日志记录该回调返回 404。
...
- **修复**：支付通知控制器同时支持 GET 和 POST；GET 请求将查询参数交给渠道 Adapter 验签，POST 保留原始请求体处理。
- **验证计划**：本地执行支付控制器单测、后端全量测试和构建；推送后在 `lch:/root/lumina` 拉取、重建 backend，并使用真实支付回调或受控签名回调确认订单变为 `SUCCEEDED`、回调事件标记已处理且余额入账。
- **优先级**：P0
- **状态**：代码修复中；远程尚未部署本次修复。
