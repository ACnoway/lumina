...
### PAYMENT-001 · 易支付回调方式与查单兼容问题导致充值不入账（已修复，待重新部署验证）
...
- **验证计划**：本地执行支付控制器、易支付 Adapter 单测、后端全量测试和构建；推送后在 `lch:/root/lumina` 拉取、重建 backend，并重放该笔已由易支付查单确认成功的通知，确认订单变为 `SUCCEEDED`、回调事件标记已处理且余额入账。
- **优先级**：P0
- **状态**：代码修复中；GET 路由已部署，查单兼容和安全兜底待本次提交部署验证。
